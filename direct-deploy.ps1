# Direct Deployment Script - Bypasses npm build process
Write-Host "Creating a direct deployment version..." -ForegroundColor Cyan

# Create a new simplified index.html
$indexHtml = @'
<!DOCTYPE html>
<html lang="en">
<head>
...
# Create a blank build.sh file to override any npm scripts
$buildSh = @'
#!/bin/bash
echo "Static deployment - no build needed"
exit 0
'@

Set-Content -Path "build.sh" -Value $buildSh
if ($PSVersionTable.Platform -ne "Unix") {
    # Convert to Unix line endings for bash scripts
    (Get-Content "build.sh") | ForEach-Object { $_ -replace "`r`n", "`n" } | Set-Content "build.sh" -NoNewline
}

Write-Host "`nDirect deployment files created!" -ForegroundColor Green
Write-Host "Now run:`n" -ForegroundColor Yellow
