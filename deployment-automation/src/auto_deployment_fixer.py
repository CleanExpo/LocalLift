...
def generate_summary(endpoints, fixes_applied, verification_results):
    ...
    with open(summary_file, "w") as f:
        ...
        if fixes_applied.get("port_fixed"):
            f.write("✅ PORT configuration fixed in main.py\n")
        else:
            f.write("❌ PORT configuration was not fixed\n")
        
        if fixes_applied.get("config_updated"):
            f.write("✅ API endpoint updated in config.js\n")
        else:
            f.write("❌ API endpoint was not updated in config.js\n")
        
        f.write("\nVERIFICATION RESULTS:\n")
        f.write("--------------------\n")
        if verification_results.get("railway"):
            f.write("✅ Railway backend is accessible\n")
        else:
            f.write("❌ Railway backend is not accessible\n")
        
        if verification_results.get("vercel"):
            f.write("✅ Vercel frontend is accessible\n")
        else:
            f.write("❌ Vercel frontend is not accessible\n")
        
        if verification_results.get("supabase"):
            f.write("✅ Supabase database configuration found\n")
        else:
            f.write("❌ Supabase database configuration not found\n")
