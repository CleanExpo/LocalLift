...
function Run-EndpointDiscovery {
    ...
    try {
        python mcp_endpoint_discovery.py | Tee-Object -FilePath "$LogDir\endpoint-discovery-$Timestamp.log" -Append
...
function Run-AutoDeploymentFixer {
    ...
    try {
        python auto_deployment_fixer.py @args | Tee-Object -FilePath "$LogDir\auto-fixer-$Timestamp.log" -Append
