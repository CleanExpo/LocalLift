...
if (-not (Get-Command vercel -ErrorAction SilentlyContinue)) {
    Write-Host "X Vercel CLI not found. Please install it with: npm install -g vercel" -ForegroundColor Red
    exit 1
}
...
try {
    ...
    Write-Host "X Not logged in to Vercel. Please login with: vercel login" -ForegroundColor Red
    exit 1
}
...
if (-not (Test-Path "public")) {
    Write-Host "X The public directory does not exist. Please create it before deploying." -ForegroundColor Red
    exit 1
}

# Check for index.html in public directory
if (-not (Test-Path "public/index.html")) {
    Write-Host "X public/index.html not found. This is required for the SPA." -ForegroundColor Red
    exit 1
}

# Verify vercel.json configuration
if (-not (Test-Path "vercel.json")) {
    Write-Host "X vercel.json not found. This is required for configuration." -ForegroundColor Red
    exit 1
}
